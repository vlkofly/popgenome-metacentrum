...
cp /storage/brno3-cerit/home/vlkofly/mockingbird_genomics/programs/twisst/plot_twisst_v1local.R $SCRATCHDIR
...
### plot results at least the summary of first 5 trees

zcat ${base}.weights.csv.gz | cut -f 1-6 | gzip -c > ${base}.6topo.weights.csv.gz
...
cat << EOF > script.R
source("plot_twisst_v1local.R")
weights_file <- "${base}.6topo.weights.csv.gz"
window_data_file <- "ne" 
twisst_data <- import.twisst(weights_files=weights_file,
                             window_data_files=window_data_file)
svg("${base}.twisst.sumplot.svg")
