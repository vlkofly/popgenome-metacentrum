...
nt='1'
#vcf="missing0.85_depth8to150.recode.vcf" # 
#ref="Toxostoma_redivivum.genome.fa"
wd=$PBS_O_WORKDIR
sn="/storage/brno3-cerit/home/vlkofly/mockingbird_genomics/sample_lists/renaming.old.new.melan.txt" # list of old and new names 
batch=melan #name of output folder
dp_path=$wd/$dp
dp_base=`basename $dp`
...
# filter blackmask and Z chromosome
vcftools --gzvcf $vcf_base --not-chr Mmel_ChrZ  --exclude-bed $dp_base --minDP 7 --recode --max-missing 0.75 --recode-INFO-all --stdout | bgzip -c > ${base}.masked.mindp7.0.75missing.noZ.vcf.gz 
vcf=${base}.masked.mindp7.0.75missing.noZ.vcf.gz
tabix -p vcf $vcf

#vcftools --vcf $vcf $chrlist --LROH --out allchr1Msubset_depth10to30 # another option for calling ROH in vcftools
